...
export default function MenuScreen() {
  ...
  const handleLogout = async () => {
    ...
    //   CommonActions.reset({
    //     index: 0,
    //     routes: [{ name: 'Auth' }], // En RootNavigator el stack de login es "Auth"
    //   })
    // );
  };
  ...
  const handleGoToCertificates = () => {
    navigation.navigate('Main', {screen: 'Certificates'}); // Está en MainStack
  };

  const handleGoToEvaluations = () => {
    navigation.navigate('Main', {screen: 'Evaluaciones'}); // Está en MainStack
  };

  const handleGoToProjects = () => {
    navigation.navigate('Main', {screen: 'MisProyectos'}); // Está en MainStack
  };
    return (
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <Image
            source={{ uri: 'https://classic.battle.net/war3/images/neutral/units/portraits/pandarenbrewmaster.gif' }}
            style={styles.logo}
          />
        </View>
